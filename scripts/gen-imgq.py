...
parser.add_argument('--cathode', required=True, type=float, help='location (z coordinate) of cathode')
parser.add_argument('--anode', type=float, help='location (z coordinate) of anode if using Petersen\'s method')
...
for ii, id in enumerate(id_atoms):
    ...
    frame_out.positions[ii + len(frame.positions)][2] = 2 * args.cathode - frame.positions[id][2]
...
if args.anode is not None:
    for ii, id in enumerate(id_atoms):
        ...
        frame_out.positions[ii + len(frame.positions) + len(id_atoms)][2] = 2 * args.anode - frame.positions[id][2]
