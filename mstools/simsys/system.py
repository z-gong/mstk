...
class System():
    ...
    def extract_required_ff_terms(self):
        ...
        self.type_names: [str] = []
        # all required self-self and pairwise vdW terms, excluding those generated by mixing rule
        self.vdw_terms: [VdwTerm] = []
        ...
        for bond in self._topology.bonds:
            if bond.is_drude:
                ...
                continue
            try:
                bterm = self._params.get_bond_term(bond)
        ...
        for angle in self._topology.angles:
            try:
                aterm = self._params.get_angle_term(angle)
            except Exception as e:
        ...
        for dihedral in self._topology.dihedrals:
            try:
                dterm = self._params.get_dihedral_term(dihedral)
            except Exception as e:
        ...
        for improper in self._topology.impropers:
            try:
                iterm = self._params.get_improper_term(improper)
            except Exception as e:
            ...
            if vdw_class == LJ126Term:
                ...
                for i in range(n_type):
                    for j in range(n_type):
                        vdw = self._params.get_vdw_term(self.atom_types[i], self.atom_types[j])
